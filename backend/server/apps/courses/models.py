...
class Category(MPTTModel):
    ...
    name = models.CharField(
        _("Nombre"), max_length=100,
        help_text=_("format: required, max-100"))
    slug = models.SlugField(
        _("URL segura de categoria"),
        max_length=150,
...
class Classroom(TimestampMixin):
    ...
    internal_reference = models.CharField(
        _('Código Interno'), max_length=120, unique=True)
    available_vacancie = models.IntegerField(
        _('Vacantes disponibles'), default=0)
    max_vacancies = models.IntegerField(
        _('Vacantes Máximas'), default=0)
    student_enrollment = models.IntegerField(
        _('Alumnos Matriculados'), default=0)
    course = models.ForeignKey(
        Course, on_delete=models.CASCADE,
        related_name='classroom', related_query_name='classroom')
    start_hour = models.CharField(
        _('Hora de Inicio'), max_length=15)
    end_hour = models.CharField(
        _('Hora de Fin'), max_length=15)
    qty_session = models.IntegerField(
        _('Cantidad de Sesiones'), default=0)
    is_active = models.BooleanField(
        default=True,
